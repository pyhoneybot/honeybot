...
class Plugin:
    ...
    def count_color_property(portfolio, col):
        ...
        properties = [
            asset for asset in portfolio if isinstance(asset, monopoly_assets.Property)
    ...
    def count_utilities(portfolio):
        ...
        return len(
            [utility for utility in portfolio if isinstance(utility, monopoly_assets.Utility)]
        )

    def count_railroads(portfolio):
        """counts number of railroads in a given portfolio"""
        return len(
            [railroad for railroad in portfolio if isinstance(railroad, monopoly_assets.Railroad)]
        )
    ...
    def show_rolls(methods, info, player):
        ...
        for i in range(2, 13):
            ...
            if index > 39:
                index -= 40
            space = monopoly_assets.board_spaces[index]
            if (
                isinstance(space, monopoly_assets.Property)
                or isinstance(space, monopoly_assets.Railroad)
                or isinstance(space, monopoly_assets.Utility)
            ):
                ...
                if owner_info[0]:
                    if owner_info[1] == player:
                        ...
                        if isinstance(space, monopoly_assets.Property):
                            message = (
                                message
    ...
    def get_player_info(methods, info, player):
        ...
        if player.getPosition() == 10 and player.imprisoned:
            ...
            location = monopoly_assets.board_spaces[player.getPosition()].get_name()
            methods["send"](
                info["address"],
                name
                + " is at "
    ...
    def get_help(methods, info):
        ...
        methods["send"](
            info["address"],
            "'.monopoly buy' buys the property where you are on the board "
            "or buy a house for the property.",
        )
        methods["send"](
            info["address"],
    ...
    def get_rent(asset, asset_owner, move_amount):
        ...
        if isinstance(asset, monopoly_assets.Property):
            return asset.rents[asset.house_count]

        elif isinstance(asset, monopoly_assets.Utility):
            num_util = len(
                [
                    property
                    for property in asset_owner.getPortfolio()
                    if isinstance(property, monopoly_assets.Utility)
                ]
            )
            print(num_util)
        ...
        elif isinstance(asset, monopoly_assets.Railroad):
            num_rail = len(
                [
                    property
                    for property in asset_owner.getPortfolio()
                    if isinstance(property, monopoly_assets.Railroad)
                ]
            )
            print(num_rail)
    ...
    def move_to_space(methods, info, player, space):
        ...
        if monopoly_assets.board_spaces.index(space) < player.getPosition() <= 39:
            methods["send"](
                info["address"],
                player.getName() + " passed go moving to " + space.get_name(),
            )
            player.increasePot(200)
            player.setPosition(monopoly_assets.board_spaces.index(space))
        elif player.getPosition == monopoly_assets.board_spaces.index(space):
            methods["send"](
                info["address"],
    ...
    def get_location_info(methods, info, player, location):  # location is a space object
        ...
        """
        if isinstance(location, monopoly_assets.Property):
            methods["send"](info["address"], location.info())
            methods["send"](
                info["address"],
                "Remember that you need a full set to buy houses! "
                "Of the other properties in the set:",
            )
            set_others = []
            for space in monopoly_assets.board_spaces:
                if isinstance(space, monopoly_assets.Property):
            ...
            for space in set_others:
                ...
                if owner_info[0]:
                    if owner_info[1].getName() == player.getName():
                        ...
                        methods["send"](
                            info["address"],
                            space.get_name() + " is owned by " + owner_info[1].getName(),
                        )
                else:
                    methods["send"](info["address"], space.get_name() + " is unowned!")

        elif isinstance(location, monopoly_assets.Railroad):
            methods["send"](info["address"], location.info())
            other_rails = []
            for space in monopoly_assets.board_spaces:
                if isinstance(space, monopoly_assets.Railroad) and space != location:
        ...
        elif isinstance(location, monopoly_assets.Utility):
            methods["send"](info["address"], location.info())
            for space in monopoly_assets.board_spaces:
                if isinstance(space, monopoly_assets.Utility) and space != location:
                    other_util = space
                    break
            ...
            if owner_info[0]:
                if owner_info[1].getName() == player.getName():
                    ...
                    methods["send"](
                        info["address"],
                        other_util.get_name() + " is owned by " + owner_info[1].getName(),
                    )
            else:
                methods["send"](info["address"], other_util.get_name() + " is unowned!")
        else:
            methods["send"](info["address"], "No special information can be given for this space!")
    ...
    def pay(methods, info, payer, receiver, amount):
        """payers pays a user a certain amount and
        may be kicked out of the game if has no money"""
    ...
    def offer_house(methods, info, player, property):
        ...
        if num_prop_in_set == monopoly_assets.Property.set_houses[property.color]:
    ...
    # additional arguments supplied can be found in the second element of the values
    # of the dictionaries of the monopoly_assets.py file.
    # here's an example
    # {"From sale of stock, you get $45":[0,(45)]}
    # 0 refers to earn as it is the function at first index of community_deck_methods
    # and (45) is the argument.
    ...
    def community_card(methods, info, player):
        ...
        # take community Card
        # must pick first in array, place it at back,
        # and execute its function with correct args.
    ...
    def chance_card(methods, info, player):
        ...
        card = monopoly_assets.chance_deck.pop(
            0
        )  # take first card, returns a dict {"message":[int,(tuple)]}
        card_string = next(iter(card))
        methods["send"](info["address"], card_string)
        ...
        func(methods, info, player, *card[card_string][1])
        monopoly_assets.chance_deck.append(card)  # put it at the back
    ...
    def collect(methods, info, player, amount):
        """collect amount from all players,
        can't use def pay as will execute next_turn too many times"""
    ...
    def jail(methods, info, player):
        """
        remove get out of jail card if player has it,
        otherwise nd to jail, do not gain 200
        """
        if player.get_outta_jail_card:
            methods["send"](
                info["address"],
                player.getName() + " is at jail but used his get out of jail free card!",
            )
    ...
    def repairs(methods, info, player, house_fee, hotel_fee):
        ...
        property_list = [
            asset for asset in player.getPortfolio() if isinstance(asset, monopoly_assets.Property)
        ]
        hotel_total = 0
        house_total = 0
    ...
    def pay_all(methods, info, player, amount):
        """check if player can pay the amount, remove if not,
        and increment pot of each opponent"""
    ...
    def reading_rail(methods, info, player):
        """move player to reading railroad and check if owned by player,
        owned by opponent or unowned"""
        reading_rail = monopoly_assets.board_spaces[5]
    ...
    def railroad(methods, info, player):
        """move to nearest railroad and check if owned by player,
        owned by opponent or unowned"""
        # find railroad, all railroads have index ending in five.
        # in addition, player must move forwards.
        ...
        for i in range(10):  # player can only be 10 away from next railroad
            ...
            if isinstance(
                monopoly_assets.board_spaces[player.getPosition()],
                monopoly_assets.Railroad,
            ):
                break
        railroad = monopoly_assets.board_spaces[player.getPosition()]
        owner_info = Plugin.find_owner(railroad.get_name())
        if owner_info[0]:
            if owner_info[1] == player:
                methods["send"](
                    info["address"],
                    player.getName() + " is at " + railroad.get_name() + " which they already own",
                )
                Plugin.next_turn()
            else:
                owner_railroads = Plugin.count_player_properties(owner_info[1])["Railroad"]
    ...
    def move_to_property(methods, info, player, property):
        """move to specific property and and check if owned by player,
        owned by opponent or unowned"""
    ...
    def util(methods, info, player):
        """
        move to nearest utility and and check if owned by player,
        owned by opponent or unowned (should pay double rent)
        """
        ...
        while (
            not (
                isinstance(
                    monopoly_assets.board_spaces[player.getPosition()],
                    monopoly_assets.Utility,
                )
            )
        ) and start:
            start = True
            player.updatePosition(1)
        utility = monopoly_assets.board_spaces[player.getPosition()]
        ...
        if owner_info[0]:
            if owner_info[1] == player:
                ...
                methods["send"](
                    ...
                    + " has landed on "
                    + owner_info[1].getName()
                    + "'s utility and must pay ten times"
                    + " whatever he rolls now with two dice!",
                )
    ...
    def move_back_three(methods, info, player):
        ...
        if player.getPosition == 22:
            ...
            property = monopoly_assets.board_spaces[19]
            owner_info = Plugin.find_owner(property.get_name())
            if owner_info[0]:
                if owner_info[1] == player:
                    Plugin.offer_house(methods, info, player, property)
    ...
    def create(methods, info, name):
        """check if game exists, otherwise create game,
        require join command rather than create command"""
    ...
    def join(nickname):
        """add player to game if a game has been created
        and the player is not already a player"""
        if Plugin.stage == 0 and Plugin.start_join_req:
            nicknames = [player.getName() for player in Plugin.players]
            if nickname not in nicknames:
                Plugin.players.append(monopoly_assets.Player(nickname))
    ...
    def start(methods, info):
        ...
        if Plugin.start_join_req:
            ...
            random.shuffle(monopoly_assets.community_deck)
            random.shuffle(monopoly_assets.chance_deck)
        else:
            methods["send"](info["address"], "This command is currently not possible")

    def roll(methods, info):
        """
        roll dice to move player, unless player in prison or its already game over,
        and check new position
        """
        if Plugin.roll_req and not Plugin.game_over:
            ...
            if not player.imprisoned or player.prison_time == 2:  # if user will move for sure
                ...
                if (
                    ...
                    player.imprisoned = False  # should free the player regardless
                    # of whether a double is thrown
                    player.prison_time = 0
                    if roll1 == roll2:
                        methods["send"](
                            info["address"],
                ...
                new_location = monopoly_assets.board_spaces[player.getPosition()]
                methods["send"](
                    info["address"],
                    player.getName()
                    + " rolled "
                ...
                if (
                    isinstance(new_location, monopoly_assets.Property)
                    or isinstance(new_location, monopoly_assets.Railroad)
                ) or isinstance(new_location, monopoly_assets.Utility):
                ...
                if roll1 == roll2:
                    ...
                    else:  # don't need to check other options as we know user starts at
                        # index ten and throwing a double limits the possibilities
                        Plugin.next_turn()
    ...
    def buy(methods, info):
        ...
        if Plugin.buy_pass_req:
            ...
            Plugin.roll_req = True
            player = Plugin.players[Plugin.turn]
            property = monopoly_assets.board_spaces[player.getPosition()]
            owner_info = Plugin.find_owner(property.get_name())
            if owner_info[0]:
                if owner_info[1].getName() == player.getName():  # if owner is user
                    ...
                    if int(player.getPot()) > property.house_cost:
                        if (
                            ...
                            == monopoly_assets.Property.set_houses[property.color]
                        ):  # if full set
                ...
                if int(player.getPot()) > property.price:
                    ...
                    if isinstance(property, monopoly_assets.Property) or isinstance(
                        property, monopoly_assets.Railroad
                    ):
                        methods["send"](
                            info["address"],
                            player.getName()
                        ...
                        Plugin.next_turn()
                    elif isinstance(property, monopoly_assets.Utility):
                        if Plugin.count_player_properties(player)["utility"] == 1:
                            methods["send"](
                                ...
                                + " for "
                                + str(property.price)
                                + " and if you land there you will have "
                                + "to pay four times whatever you roll!",
                            )
                        else:
                            methods["send"](
                                info["address"],
                                ...
                                + str(property.price)
                                + " and if you land there you will have "
                                + "to pay ten times whatever you roll!",
                            )
                else:
                    methods["send"](
                        info["address"],
        ...
        try:
            if info["command"] == "PRIVMSG":
                ...
                if msgs[0] == ".monopoly":
                    ...
                    if not (
                        Plugin.checkWon(methods, info)
                    ):  # if Plugin.checkWon evaluates true it will print winner already
                        if len(msgs) == 1:
                            methods["send"](
                                info["address"],
                                ".monopoly requires an argument:"
                                + " create, join, start, roll, buy, "
                                + "pass, info, help or leave",
                            )
                            ...
                            if msgs[1] == "info":
                                if msgs[2] == "rolls":
                                    ...
                                    if isinstance(player, monopoly_assets.Player):
                                            ...
                                            if isinstance(
                                                ...
                                                if owner_info[0]:
                                                    if owner_info[1].getName() == pl.getName():
                                                        methods["send"](
                                                            info["address"],
                                                            "You already own " + space.get_name(),
                                                        )
                                                    else:
                                                        methods["send"](
                                                            info["address"],
                                                ...
                                                methods["send"](
                                                    info["address"],
                                                    "You are not playing and so can not "
                                    ...
                                    if prop is False:
                                        ...
                                        if isinstance(pl, monopoly_assets.Player):
                                ...
                                methods["send"](
                                    ...
                                    + " for a third argument which can be rolls, "
                                    + "a player name or a property name."
                                    + " Try '.monopoly info rolls'",
                                )
